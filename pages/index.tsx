...
// (The 'allTestimonials' list remains the same)
const allTestimonials = [
  ...
  { name: 'David L.', country: 'Australia', flag: '🇦🇺', comment: 'I was new to investing, but this platform made it easy to get started.', imageUrl: 'https://i.pravatar.cc/150?img=7' },
  { name: 'Fatima A.', country: 'UAE', flag: '🇦🇪', comment: 'The ROI on the Platinum Plan has exceeded all my expectations.', imageUrl: 'https://i.pravatar.cc/150?img=8' },
  { name: 'Carlos R.', country: 'Brazil', flag: '🇧🇷', comment: 'QuantumLeap has provided consistent returns and their support team is always helpful.', imageUrl: 'https://i.pravatar.cc/150?img=11' },
  { name: 'Chloe B.', country: 'France', flag: '🇫🇷', comment: 'Secure, reliable, and profitable. What more could you ask for in an investment platform?', imageUrl: 'https://i.pravatar.cc/150?img=12' },
  { name: 'Wei Z.', country: 'China', flag: '🇨🇳', comment: 'The market analysis tools are incredibly insightful.', imageUrl: 'https://i.pravatar.cc/150?img=14' },
  { name: 'Olivia P.', country: 'Canada', flag: '🇨🇦', comment: 'I have tried many platforms, and this is by far the most user-friendly and effective.', imageUrl: 'https://i.pravatar.cc/150?img=15' },
];

// We will also make the amounts and plans more random
const plans = ['Silver', 'Gold', 'Platinum', 'Diamond'];
const amounts = [500, 1200, 2500, 5000, 10000, 750, 15000];

export default function HomePage() {
  // State to hold the user data we fetch from the API
  ...
  // This useEffect fetches the random user data when the page loads
  useEffect(() => {
    fetch('https://randomuser.me/api/?results=10&nat=us,gb,ca,au,de,fr,jp,br')
      .then(res => res.json())
      .then(data => {
        const formattedUsers = data.results.map((user: any) => ({
          name: `${user.name.first} ${user.name.last.charAt(0)}. from ${user.location.country}`
        }));
        setInvestors(formattedUsers);
      });
  }, []);

  // This useEffect handles the daily testimonials (no change here)
  useEffect(() => {
    const getDaySeed = () => { const date = new Date(); return date.getFullYear() * 1000 + date.getMonth() * 100 + date.getDate(); };
    const shuffle = (array: any[], seed: number) => { let currentIndex = array.length, temporaryValue, randomIndex; let random = () => { var x = Math.sin(seed++) * 10000; return x - Math.floor(x); }; while (0 !== currentIndex) { randomIndex = Math.floor(random() * currentIndex); currentIndex -= 1; temporaryValue = array[currentIndex]; array[currentIndex] = array[randomIndex]; array[randomIndex] = temporaryValue; } return array; };
    const seed = getDaySeed();
    const shuffled = shuffle([...allTestimonials], seed);
    setDailyTestimonials(shuffled.slice(0, 3));
  }, []);

  // This useEffect now uses the API data for the news ticker
  useEffect(() => {
    // Don't start the timer until we have investor data from the API
    if (investors.length === 0) return;

    const interval = setInterval(() => {
      // Pick a random investor from our new list
      const randomInvestor = investors[Math.floor(Math.random() * investors.length)];
      // Pick a random amount and plan
  ...
  }, [investors]); // This effect will re-run once the 'investors' array is filled
  ...
  return (
    <div style={{ backgroundColor: '#111827', color: 'white' }}>
      ...
      {/* The ticker now uses fully dynamic, real-looking data */}
